...
DEFAULT_REDIRECT_URL = 'https://internship-pro.onrender.com/linkedin/callback'
...
def verify_email_otp(request):
    """
    Verify the OTP sent to email using only EmailVerification model
    """
    if request.method != 'POST':
        return JsonResponse({"status": "error", "message": "Only POST method allowed"}, status=405)
    ...
    try:
        ...
        try:
            ...
            if str(verification.otp) == str(otp_submitted):
                verification.is_verified = True
                  # only if created_at allows null=True
                verification.save()
...
def register(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # Accept this flag from the frontend to know they clicked a paid plan
            came_from_plan = data.get('came_from_plan', False)
            ...
            user_extra_fields = {
                ...
                "email_verified": True,  # Set to False initially in production
            }
            if came_from_plan:
                user_extra_fields["tag"] = "Registered through membership plan"
    ...
    try:
        ...
        try:
            ...
            return JsonResponse({
                ...
                "role":user.role,
                "message": (
                    f"Email already registered. Please login. "
                    f"(email_verified: {user.email_verified}, paid: {user.paid}, role: {user.role})"
                    if user.email_verified 
                    else 
                    f"Email registered but not verified. "
                    f"(email_verified: {user.email_verified}, paid: {user.paid}, role: {user.role})")
            })
        except CustomUser.DoesNotExist:
            return JsonResponse({
...
DOMAIN = 'https://internship-pro.onrender.com'
class CreateCheckoutSessionView(APIView):
    ...
    def post(self, request):
        try:
            ...
            # Convert amount safely to int (cents)
            try:
            ...
            checkout_session = stripe.checkout.Session.create(
                ...
                line_items=[
                    {
                        "price_data": {
                            ...
                            "product_data": {
                                "name": product_name,
                                "description": f"Plan purchased by {full_name}",
                            },
                        },
                        "quantity": 1,
                    }
                ...
                mode="payment",
                success_url=f"{getattr(settings, 'FRONTEND_URL', 'https://prspera.com')}/payment-success?session_id={{CHECKOUT_SESSION_ID}}",
                # success_url=DOMAIN + '/success',
                cancel_url=f"{getattr(settings, 'FRONTEND_URL', 'https://prspera.com')}/payment-cancelled",
                metadata={
                    "user_id": str(request.user.id),
            ...
            StripePayment.objects.create(
                ...
                email=email,
                amount=float(amount_cents) / 100,  # Convert cents → dollars
                currency="usd",
                status="pending",
                customer_name=full_name,
                company_name=billing_info.get("companyName", ""),
            ...
            logger.info(
                f"Checkout session {checkout_session.id} created for user {request.user.id}"
            )
...
class ProcessPaymentView(APIView):
    ...
    def post(self, request):
        try:
            ...
            email = billing_info.get('email')
            try:
                logger.info(f"Searching Stripe customer by email: {email}")
                customers = stripe.Customer.list(email=email, limit=1)
                if customers.data:
                    customer = customers.data[0]
                    logger.info(f"Found existing Stripe customer: {customer.id}")
                else:
                    logger.info(f"Creating new Stripe customer with email: {email}")
                    customer = stripe.Customer.create(
                        email=email,
                ...
                logger.error(f"Stripe error creating/finding customer: {e.user_message or str(e)}")
                return Response({
                    'error': f'Error processing customer information: {e.user_message or "Unknown error"}'
                }, status=status.HTTP_400_BAD_REQUEST)
            ...
            try:
                ...
                logger.error(f"Stripe error creating payment method: {e.user_message or str(e)}")
                return Response({
                    'error': f'Invalid payment method: {e.user_message or str(e)}'
                }, status=status.HTTP_400_BAD_REQUEST)
            ...
            try:
                ...
                logger.error(f"Card declined: {e.user_message or str(e)}")
                return Response({
                    'error': f'Payment declined: {e.user_message or str(e)}'
                }, status=status.HTTP_400_BAD_REQUEST)

            except stripe.error.StripeError as e:
                logger.error(f"Stripe error during payment intent creation: {e.user_message or str(e)}")
                return Response({
                    'error': f'Payment processing error: {e.user_message or str(e)}'
                }, status=status.HTTP_400_BAD_REQUEST)
...
from .serializers import UserOverviewSerializer
class AllUsersOverviewAPIView(APIView):
